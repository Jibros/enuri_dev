...
//	detailWin.focus();
	...
	*/
	...
	detailWin = window.open("/_pre_detail_.jsp","detailMultiWin","width=804,height="+window.screen.height+",left=0,top=0,toolbar=no,directories=no,status=no,scrollbars=yes,resizable=yes,menubar=no");
...
function copyCatalogUrl(modelno,userid,imgtype,widthtype,ecatalmodel) {
	...
	if(ecatalmodel>0 && imgtype==4){ //큰사진만 퍼가기는 다르게
		...
		*/
		...
		var returnImageUrl = "";
		if (widthtype != "3"){
			returnImageUrl = "http://catalog.enuri.gscdn.com/func_info/"+userid+"/"+modelno+"_"+imgtype+withtype_name+".png";
		}else{
			returnImageUrl = "http://goodsinfo.enuri.gscdn.com/func_info/"+userid+"/"+modelno+"_"+imgtype+withtype_name+".png";
		}
...
function changeRecentZzim(type){
	if (type == 1){
		RB_BUTTONS_HEIGHT = 34;
		RB_TOP = 509;
		document.getElementById("rb_buttons").className = "rb_buttons1";
		document.getElementById("mytoday_btn").src = "http://img.enuri.gscdn.com/images/main/2015/hp/rvi_recent_item.gif";
		document.getElementById("mysave_btn").src = "http://img.enuri.gscdn.com/images/main/2015/hp/rvi_zzim.gif";
		if (document.getElementById("mysave_cnt").innerHTML.trim().length == 0){
			document.getElementById("mysave_btn").style.marginTop = "5px";
		}else{
			document.getElementById("mysave_btn").style.marginTop = "0px";
		}
		if(!(BrowserDetect.browser == "Explorer" && BrowserDetect.version == 6)) {
			if (BrowserDetect.OS == "iPad" && BrowserDetect.browser == "Safari"){
				document.getElementById("rb_buttons").style.top = setRbButtonsTopPositionIPad();
	...
	}else if(type == 2 && document.URL.indexOf("/view/Sslist_2013.jsp") < 0 ){
		RB_BUTTONS_HEIGHT = 21;
		RB_TOP = 517;
...
function svCheck(modelno,tp){
	if (svCheck.addModelNos){
		...
		if (varSameModelNo >= 0){
			...
			document.getElementById("sv_"+modelno).src = "http://img.enuri.gscdn.com/images/main/2015/hp/check.gif"
		}else{
			svCheck.addModelNos[svCheck.addModelNos.length] = modelno;
			svCheck.addModelType[svCheck.addModelType.length] = tp;
			document.getElementById("sv_"+modelno).src = "http://img.enuri.gscdn.com/images/main/2015/hp/check_on.gif"
		}
	}else{
...
function nvCheck(modelno,tp){
	if (nvCheck.addModelNos){
		...
		if (varSameModelNo >= 0){
			...
			document.getElementById("nv_"+modelno).src = "http://img.enuri.gscdn.com/images/main/2015/hp/check.gif"
		}else{
			nvCheck.addModelNos[nvCheck.addModelNos.length] = modelno;
			nvCheck.addModelType[nvCheck.addModelType.length] = tp;
			document.getElementById("nv_"+modelno).src = "http://img.enuri.gscdn.com/images/main/2015/hp/check_on.gif"
		}
	}else{
...
function showEbaySponsorGoodsImg(k){
	if (document.URL.indexOf("/search/Searchlist.jsp") >= 0){
		if (document.getElementById("searchListFrame").contentWindow.document.getElementById("ebayOrgImg_"+k).src != "http://img.enuri.gscdn.com/images/blank.gif"){
		...
		if (document.getElementById("enuriMenuFrame").contentWindow.document.getElementById("enuriListFrame").contentWindow.document.getElementById("ebayOrgImg_"+k).src != "http://img.enuri.gscdn.com/images/blank.gif"){
