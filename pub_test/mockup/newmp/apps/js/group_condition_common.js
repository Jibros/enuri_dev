...
var headerComponent = "";
    ...
    headerComponent += "  <h1>NEW MP : 그룹조건 관리</h1>";
...
*********************************/
var layout = new dhx.Layout("groupConditionLayout", config);

    layout.getCell("sidebar").attach(sidebar);
    // GRID 세팅
    layout.getCell("groupMdGrid").attach(groupMdGrid); // 그룹모델 판매조건 GRID
    layout.getCell("groupMdHeader").attach(groupMdHeader); // 그룹모델 판매조건 헤더
