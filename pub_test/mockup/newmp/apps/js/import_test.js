...
import { sidebar } from  "./layout/left_category.js" // 사이드 메뉴
import * as header from "./layout/top_header.js"
...
var layout = new dhx.Layout("layout", config);
    ...
    // GRID 세팅
    layout.getCell("mkrgrid").attach(grid1); // 제조사 GRID
    layout.getCell("brndgrid").attach(grid2); // 브랜드 GRID
...
*********************************/
    ...
    grid1.selection.events.on("afterSelect", function(row, col){
        layout.getCell("col-1").config.header="제조사 (3,808) : " + row.gcol1;

        layout.paint();
    });
    // 브랜드 선택 -> header 변경
    grid2.selection.events.on("afterSelect", function(row, col){
        layout.getCell("col-2").config.header="브랜드 (126) : " + row.gcol1;
