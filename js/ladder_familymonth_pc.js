$(function(){
    ...
    var heightNode = 5, // 사다리 세로 갯수		
		...
		heightNum = 56, // 각 사다리 높이
		widthNum = 210, // 각 사다리 너비
		borderThick = 22; // 보더 두께
	...
	function rewards(result){
		if(result >= 0){
			 // 당첨 레이어 오픈
            $("#prizes").stop().delay(500).fadeIn(300, function(){
        ...
        if(y ==heightNode ){ // 사다리 결과 
			...
			$('#' + node + "-user").animate({opacity:0}, 300, function(){
				$(this).stop().animate(
					{opacity:1}, 500)
					.addClass("open")
					.animate({bottom: '-95px'}, 400)
					.animate({bottom: '-105px'}, 400)
                ...
                getEventAjaxData( {"procCode":1} , function(data) {
				    ...
				    if(result >= 0){
						...
						alert("이미 참여하셨습니다. 다음 이벤트에 다시 참여해주세요.");
