...
<%
	...
	String strStartDate = "20220704"; //실서버 반영시 20220704로 변경
	...
	try{
		if (intGetType > 0) {
			if (intGetType == 1) { //응모권 개수
				intResult = Event_Newsemester_Proc.newGetEventTicketCnt(userId, strToday); //이벤트 기간 내 응모권 개수 >> 실서버 배포할때도 이걸로
				resultObj.put("result", intResult);
			} else if(intGetType == 2) { //상품잔여개수 
				String strEventId = "2022070401,2022070402,2022070403,2022070404,2022070405,2022070406";
				...
				ticketObj = Event_Newsemester_Proc.newGetTodayTicketData(strToday, userId); //1,2,3,5번 응모권
