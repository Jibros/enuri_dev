...
// getParameter
	...
	try {
		if(referer.indexOf("enuri.com") > -1){
			...
			if(!"".equals(sdt)  && referer.indexOf("dev") > -1 ){
				strToday = sdt;
			}
			...
			if(intToday >= 20181101 && intToday <= 20181130) {
				strEventId = "2018110107";//10월 보너스해택 이벤트코드
				chkEventId = "2018100104";//9월 매일체크 이벤트코드
			}else if(intToday >= 20181201 && intToday <= 20181231) {
				strEventId = "2018120307";//12월 보너스해택 이벤트코드
				chkEventId = "2018110304";//11월 매일체크 이벤트코드				
			}else if(intToday >= 20190101 && intToday <= 20190131) {
				strEventId = "2019010107";//2019년 01월 보너스해택 이벤트코드
				chkEventId = "2018120304";//2019년 12월 매일체크 이벤트코드			
			}else if(intToday >= 20190201 && intToday <= 20190228) {
				strEventId = "2019010207";//2019년 02월 보너스해택 이벤트코드
				chkEventId = "2019010104";//2019년 01월 매일체크 이벤트코드			
			}
			
		    if (strOsType.length() > 0 && cUserId.length() > 0 && chkEventId.length() > 0 && strEventId.length() > 0) {
            	Event_Every_Visit_Proc event_proc = new Event_Every_Visit_Proc();
            	intResultValue = event_proc.visit_bonus_event_ins(strEventId,chkEventId,cUserId,strOsType);
		    }
		}
	} catch(Exception ex) {
