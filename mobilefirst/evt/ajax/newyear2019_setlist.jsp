<%@page import="com.enuri.bean.event.every.Event_201608_Chuseok_Proc"%>
...
<%
	...
	if(referer.indexOf("enuri.com") > -1){
	    ...
	    if(Integer.parseInt(strToday) < 20190101 || 20190206 < Integer.parseInt(strToday)){
			return;
	    }
	}else{
		return;
	...
	*/
	...
	String eventId = "2019010705"; 													//	이벤트코드
    ...
    try{
    	if(1 == procCode){
    		...
    		*/
        	...
        	String cardImg = StringUtils.substringAfterLast(request.getParameter("cardImg"), "/");
        	String cardMsg = ChkNull.chkStr(request.getParameter("cardMsg"),"");
        	String cardDear = ChkNull.chkStr(request.getParameter("cardDear"),"");
        	String cardFrom = ChkNull.chkStr(request.getParameter("cardFrom"),"");
        	...
        	try{
        		seqNum = event_proc.dayInsertCard(userId, eventId, cardMsg, cardDear, cardFrom, cardImg, osType);

        		//bl_insert = seqNum >= 0;
        		intResultValue = seqNum;
        	} catch(Exception ex) {
        		System.out.println(ex);
        	}
        }else if(5 == procCode){

        	Event_201608_Chuseok_Proc event_201608_Chuseok_Proc = new Event_201608_Chuseok_Proc();
        	intResultValue = event_201608_Chuseok_Proc.ins_event(eventId, userId);
