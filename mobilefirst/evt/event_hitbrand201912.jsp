...
if(
	...
	response.sendRedirect("/eventPlanZone/jsp/HitBrand_201912.jsp");
	return;
}

String EVENTID="2019121601";
...
<head>
	...
	<link rel="stylesheet" type="text/css" href="/css/event/y2019/hit_2019dec_m.css"/>
...
<div class="wrap">
	...
	<div class="visual">
		<div class="inner">
			<img src="http://img.enuri.info/images/event/2019/hit_dec/m_top.jpg" alt="2019 에누리 히트브랜드" class="imgs" />
			<div class="blind">
				<h1>2019 하반기 에누리 히트브랜드</h1>
				<p>
					2019년 하반기 소비자들에게 가장 사랑 받은 히트브랜드를 소개합니다.<br/>
					이벤트기간 : 2019.12.16 ~ 2020.01.31
				</p>
			</div>
		</div>
	</div>
			...
			<div class="bingobox">				
				...
				<div class="inner">				
					<div class="bingo__tit">
						<img src="http://img.enuri.info/images/event/2019/hit_dec/m_bingo_tit.png" alt="" />
						<h2 class="blind">OX 퀴즈 BINGO - !</h2>
						<p class="blind">BINGO 완성 횟수에 따라 응모권 받고 스페셜 경품에 도전해 보세요</p>
					</div>
					<div class="bingo__table">
						<div class="bingo__head">
							<h3 class="tit">OX BINGO</h3>
							<div class="count">오늘 응모권 : <span class="numb" id="bingoCnt">0</span></div>
						</div>
						...
						<div class="btn__group">
							<a href="javascript:///" class="btn__info" onclick="$('#joinExplainLayer').show(); return false"><img src="http://img.enuri.info/images/event/2019/hit_june/m_btn_bingo_01.png" alt="이벤트 참여방법" /></a>
							<a href="javascript:///" class="btn__caution" onclick="$('#note').show(); return false;"><img src="http://img.enuri.info/images/event/2019/hit_june/m_btn_bingo_02.png" alt="당첨 안내및 공지" /></a>
						</div>
						<!-- //이벤트 참여방법&유의사항 -->						
											...
											<ol>
												<li>			
													<ul class="txt_indent">
														<li>선정방법: 
															<ul>
																<li>- 히트브랜드 관련 OX퀴즈로 BINGO 완성 시마다<br/> 획득한 응모권으로 스페셜 경품전 응모 </li>
																...
																<li>- 당첨되지 못하신 참여자 중 2분을 추첨하여 <br/> 참여상을 증정합니다.</li>
															</ul>
														</li>
														<li>당첨자발표: <b>2020년 2월 4일</b><br />[에누리 고객센터 및 에누리 가격비교 APP&gt;당첨자발표]</li>
		...
		<div class="specialgiftbox">
			<div class="inner">
				<div class="specialgift__cont">
					<img src="http://img.enuri.info/images/event/2019/hit_dec/m_gift_img_01.jpg" alt="" />
					<h2 class="blind">획득하신 응모권으로 원하시는 경품에 응모하세요</h2>
					<ul class="btn_group">
						<li class="btn__join"><img src="http://img.enuri.info/images/event/2019/hit_dec/m_btn_gift_01.png" alt="스페셜 경품전 응모하기" /></li>
						<li class="btn__my" onclick="$('#giveaway').show(); return false;"><img src="http://img.enuri.info/images/event/2019/hit_dec/m_btn_gift_02.png" alt="나의 응모 현황" /></li>
					</ul>
				</div>
			</div>
		</div>
...
$(document).ready(function(){
    ...
    $("#Aswmsg1").click(function(e){
    	goSocces();
    });
    
	$("#Aswmsg2").click(function(e){
		goIncorrect();
    });
    ...
    $(".bingo__list.is-bg > li ").click(function(){
    	...
    	ga('send', 'event', '2019히트브랜드' ,'button', '2019하반기히트브랜드_퀴즈응모');
    	
    	$("#OXquiz").click(function(e){
    		if((e.target.className.indexOf("btn_close") < 0 && e.target.id.indexOf("btn") < 0)){
    			$("#OXquiz").hide();
    			return false
    		}
    	});
    ...
    $(".btn__my").click(function(){
    	...
    	var gourl = "/eventPlanZone/ajax/hitBrand201912_getData.jsp?procCode=7&sdt="+sdt;
    		...
    		success:function(data){
    			...
    			$.each(data.ticketList.ticketList,function(i,v){
    				tableHtml += "<tr>";
   					...
   					if	   (v.select_seq ==1 )	tableHtml +="<td>삼성 노트북 Pen S NT930SBE-K58A</td>";  	  				
    				else if(v.select_seq ==2 )	tableHtml +="<td>LG전자 트롬스타일러 S3RER</td>";
    				else if(v.select_seq ==3 )	tableHtml +="<td>다이슨 슈퍼소닉 HD03</td>";
    				else if(v.select_seq ==4 )	tableHtml +="<td>애플 에어팟 프로</td>";
    			...
    			ga('send', 'event', '2019히트브랜드' ,'button', '2019하반기히트브랜드_응모현황');
...
function goQuiz(num){
	...
	var gourl = "/eventPlanZone/ajax/hitBrand201912_getData.jsp?procCode=2&sdt="+sdt+"&num="+num;
	$.ajax({
		type:"GET",
		url: gourl,
		dataType: "JSON",
		...
		success:function(data){
			...
			ga('send', 'event', '2019히트브랜드' ,'button', '2019하반기히트브랜드_응모하기');
		}
	});
}
function getBingo(){
	
	var gourl = "/eventPlanZone/ajax/hitBrand201912_getData.jsp?procCode=5&sdt="+sdt;
...
function goHint(modelno){
	ga('send', 'event', '2019히트브랜드' ,'button', '2019하반기히트브랜드_힌트보기');
...
function goJoin(){
	...
	$(document).ready(function(){
        ...
        $(".tabBt > ul > li").click(function(){
            ...
            var fixedH = $(".tabBt").height() + $(".m_top").height();
            $('html, body').animate({scrollTop : Math.round(offset.top-fixedH) }, 400);
            
            var tabName = $(this).text();
            
            ga('send', 'event', '2019히트브랜드' ,'button', '2019하반기히트브랜드_Tab_'+tabName);
	...
	function hitProc(param, callback) {
		...
		param.sdt = sdt;
		//}
		$.getJSON("/eventPlanZone/ajax/hitBrand201912_getData.jsp", param ,callback)
		.done(function(){
			isClick = true;
		});
	}
	...
	function evtCommonChk (procCode) {
		...
		var chkDate = [
			{	"startDate" : 20191216,	"endDate" : 20200131	}
		];
		...
		if(Number(sdt) < chkDate[0].startDate) {
			...
			//Cmd_Login('event');
			location.href = "/mobilefirst/login/login.jsp?backUrl=/mobilefirst/evt/event_hitbrand201912.jsp?freetoken=event";
		} else {
			chkResult = true;
		}
		return chkResult;
		...
		$("body").on('click', '.btn.layjoin', function(event) {
			...
			if(cls == "unchk2"){//체크 안함
				...
				try{
					...
					var goUrl = "/eventPlanZone/ajax/hitBrand201912_getData.jsp?procCode=6&uphone="+uphone+"&selectItem="+_selected+"&sdt="+sdt;
...
//							var varShippingFee = totalObj[i].shipping_fee;	//배송비
	...
	function tabLoading(json,tabVal){
	    ...
	    $.each(json, function(i, v){
	    	if(i == 0){
	    		var title = "";
	    		if(tabVal == "digital")			title="디지털 수상부문";
	    		else if(tabVal == "appliance")	title="가전 수상부문";
	    		else if(tabVal == "computer")	title="컴퓨터 수상부문";
	    		else if(tabVal == "life")		title="라이프 수상부문";
	...
	function goClose(){
		ga('send', 'event', '2019히트브랜드' ,'button', '2019하반기히트브랜드_퀴즈레이어닫기');
		$('#quizLayer').hide();
		return false;
	}
...
<div id="OXquiz" class="dim" style="display:none;">
	...
	<div class="appLayer quiz">
		...
		<div class="layercont">
			...
			<ul class="choice_box">
				<li><button type="button" id="btnO"><img src="http://img.enuri.info/images/event/2019/hit_dec/m_oxselect_o.png" alt="O" /></button></li>
				<li><button type="button" id="btnX" ><img src="http://img.enuri.info/images/event/2019/hit_dec/m_oxselect_x.png" alt="X" /></button></li>
			</ul>
...
<div id="Aswmsg1" class="dim" style="display:none;">
	<div class="appLayer result">
		<h4>OX 퀴즈 BINGO</h4>
		<a href="javascript:///" class="close" onclick="javascript:goSocces()">창닫기</a>
		<div class="layercont">
			축하드립니다<br/>
			<em>정답입니다!</em>
		</div>
...
<div id="Aswmsg2" class="dim" style="display:none;">
	<div class="appLayer result">
		<h4>OX 퀴즈 BINGO</h4>
		<a href="javascript:///" class="close" onclick="javascript:goIncorrect()">창닫기</a>
		<div class="layercont">
			안타깝지만, 오답입니다.<br/>
			<em>다른 문제에 도전해 보세요!</em>
		</div>
	</div>
</div>	
<!-- // -->
...
<div id="joinExplainLayer" class="explain_layer" style="display:none;">
	...
	<div class="appLayer">
		...
		<div class="layercont">
			<img src="http://img.enuri.info/images/event/2019/hit_dec/m_explain_img.png" alt="" />
				...
				<div class="select_prize">
					...
					<ul class="prize_list">
						...
						<li class="chk">
							...
							<label for="opt1">
								<span class="thumb" onclick="event.stopPropagation();return false;">
									<img onclick="window.open('http://www.enuri.com/p/32977559','_blank');" src="http://img.enuri.info/images/event/2019/hit_dec/m_prize_img1.jpg" alt="" />
								</span>
								<p class="proname">삼성 노트북 Pen S (1명)</p>
							</label>
						</li>
						<li>
							...
							<label for="opt2">
								<span class="thumb" onclick="event.stopPropagation();return false;">
									<img onclick="window.open('http://www.enuri.com/p/12325897','_blank');" src="http://img.enuri.info/images/event/2019/hit_dec/m_prize_img2.jpg" alt="" />
								</span>
								<p class="proname">LG 의류관리기 (1명)</p>
							</label>
						</li>
						<li>
							...
							<label for="opt3">
								<span class="thumb" onclick="event.stopPropagation();return false;">
									<img onclick="window.open('http://www.enuri.com/p/40668079','_blank');" src="http://img.enuri.info/images/event/2019/hit_dec/m_prize_img3.jpg" alt="" />
								</span>
								<p class="proname">다이슨 드라이어 (1명)</p>
							</label>
						</li>
						<li>
							<input type="radio" id="opt4" class="p_rdo" name="opt" value=""> 
							<label for="opt4">
								<span class="thumb" onclick="event.stopPropagation();return false;">
									<img onclick="window.open('http://www.enuri.com/p/43182539','_blank');" src="http://img.enuri.info/images/event/2019/hit_dec/m_prize_img4.jpg" alt="" />
								</span>
								<p class="proname">애플 에어팟 프로 (3명)</p>
							</label>
						</li>
					</ul>
				</div>
	...
	<div class="appLayer mycheck">
		...
		<div class="layercont">
			...
			<div class="mycheck_box">
				<ul>
					<li class="get">
						<strong>획득 응모권 수 : <em id="totalTicket">0</em></strong>
					</li>
					<li class="remain">
						<strong>남은 응모권 수 : <em id="availableTicket" >0</em></strong>
					</li>
				</ul>
			</div>
			...
			<div class="detail_box">
					<div class="used">
						<strong>사용 응모권 수 : <em id="useTicket">0</em></strong>
					</div>
			...
			<p class="btn_close"><button type="button" onclick="$('#giveaway').hide();">확인</button></p>
		</div>
	</div>
	<!-- //popup_box -->
</div>
