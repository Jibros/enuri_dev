...
if(pdData != null){
		...
		if(strModelNosAry!=null && strModelNosAry.length>0) {
			for(int i=0; i<strModelNosAry.length; i++) {
				if(strModelNosAry[i].indexOf("G:")>-1 || (strModelNosAry[i].indexOf("G:")<0 && strModelNosAry[i].indexOf("P:")<0 && strModelNosAry[i].indexOf("M:")<0)) {
					if(tempModelnos.length()>0) tempModelnos += ",";
					tempModelnos += strModelNosAry[i].replace("G:", "");
				}
				if(strModelNosAry[i].indexOf("P:")>-1) {
					if(tempPlnos.length()>0) tempPlnos += ",";
					tempPlnos += strModelNosAry[i].replace("P:", "");
				}
				if(strModelNosAry[i].indexOf("M:")>-1) {
					if(tempMkspModelno.length()>0) tempMkspModelno += ",";
					tempMkspModelno += strModelNosAry[i].replace("M:", "");
				}
			}
		}
		...
		String sAppId = pdData.getAppid();
		
		if(sAppId.length() == 5) {
			appType = sAppId.substring(0, 1);
			appId = sAppId.substring(1);
		} 
		
		// 모델번호 삭제
		if(tempModelnos.length()>0) {
			blCheck = save_goods_proc.deleteGoodListModelno(tempModelnos, strUserId);
			min = save_goods_proc.deleteMinpriceAlarmModelno(appId, appType, tempModelnos, strUserId);
		}
		// 추가검색상품(plno)삭제
		if(tempPlnos.length()>0) {
			blCheck = save_goods_proc.deleteGoodListPlno(tempPlnos, strUserId);
			//save_goods_proc.deleteMinpriceAlarmPlno(appId, appType, tempModelnos, strUserId);
		}
		// 브랜드스토어상품(mksp_model_no)삭제
		if(tempMkspModelno.length()>0) {
			//save_goods_proc.deleteGoodListPlno(tempPlnos, strUserId);
			tempMkspModelno = tempMkspModelno.replaceAll(",","','");
			tempMkspModelno = "'"+tempMkspModelno+"'";
			blCheck = save_goods_proc.deleteGoodListMkspModelno(tempMkspModelno, strUserId);
		}
