...
<%
	/* 앱 3.4.1 부터 적용 예정 적용될시기는 누파리 라이브러리를 적용 할때 적용 됨
	 디파인 수정은 손진욱
	 누파리 라이브러리는 윤승환 */
	...
	String os = ConfigManager.RequestStr(request, "os", "");
...
<%!int Hex_IDINPUT = 0X00000000;
	...
	public JSONObject getDefineJson(String domain, String appName, int version, String os ,boolean isDev) {
		...
		defineJson.put("FOOTER_TITLE", "법적고지");
			...
			if (appName.equals("enuri")) {
				...
				for (int i = 0; i < siteNames.length; i++) {
					...
					JSONObject site = getSite(domain, siteNames[i], schemes, version, appName, os, isDev);
					sites.put(site);
				}
				...
				String[] schemes = { "etc", "re", "reCallPercent", "iosReCallPercent", "reLink", "reBypass", "renot", "reCoDomain", "ic", "icb", "n", "o", "l", "m", "j", "my",
				...
				for (int i = 0; i < siteNames.length; i++) {
					...
					JSONObject site = getSite(domain, siteNames[i], schemes, version, appName, os, isDev); // 사이트 이름으로 사이트 정보를 하나씩 가져와 site array에 append 한다.
					sites.put(site);
				}
				defineJson.put("SITES", sites); // 다 만들어진 site array를 SITES KEY에 defineJson에 넣는다.
			}else if (appName.equals("vaccine"))
			{//카드 인포가 여기있어서 백신은 이것만 선언한다
	...
	public String getParam(String domain, String siteName, String scheme, int version, String appName, String os, boolean isDev) {
		String param = null;
	...
	public JSONObject getSite(String domain, String siteName, String[] schemes, int version, String appName, String os, boolean isDev) {
		...
		try {
			...
			for (int i = 0; i < schemes.length; i++) {
				String param = getParam(domain, siteName, schemes[i], version, appName, os, isDev);
