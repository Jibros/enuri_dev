...
if(true){
	response.sendRedirect("http://m.enuri.com/mobilefirst/index.jsp#3");	
	return;
}
%>
<!DOCTYPE html>  
...
<div id="wrap">  
	...
	<section class="trend_wrap">		
		<div class="tab_trend" id="tab_trend" style="display:none;">
			<ul> 
				<li class="bar_item"><a id="bar_news">오늘의 뉴스</a></li>
				<li class="bar_item"><a id="bar_pickup">트렌드 픽업</a></li> 
			</ul>
		</div>
		<div class="tab_trend" id="tab_news" style="display:none;"> 
			<ul>
				<li class="bar_item"><a id="bar_news2">오늘의 뉴스</a></li>
				<li class="bar_item"><a id="bar_guide">구매가이드</a></li>
			</ul>
		</div>
		
		<!-- 151130 트렌드픽업 navigation -->
		<div class="trd_nav" style="display:none;">
			<button type="button" class="trd_prev" /></button>
			<button type="button" class="trd_next" /></button>
			<div class="trd_scroll">
		...
		<!-- trend_area -->	
			...
			<!-- trend_area -->
			<div class="trend_area sp_today">
				<h3 class="maintit">주제별 <span class="stress2">최신</span> 뉴스</h3>
				...
				<div class="news_recent">
					...
					<!-- recentnews_contents -->
					<ul class="recentnews_contents" id="recentnews_contents">
					</ul>
					<!-- //recentnews_contents -->
				</div>
				<!-- news_recent -->
		...
		<div id="PickupBody" class="cont_area"></div>
		
		<div id="GuideBody" class="cont_area">
			<!-- 최신추천팁 -->
			<div class="trend_area sp_today">
				<!--
				...
				<h3 class="maintit">많이 <span class="stress2">본</span> TIP</h3>
				<!-- guide news_rank -->
				<div class="news_rank" id="guide_top_list">
				</div>
				<!--//guide news_rank -->
...
$(function() {
	...
	if(vHash == "#guide" || vType == "guide"){
		...
		$("#NewsBody").hide();
		$("#PickupBody").hide();
		$(".bar_item").removeClass("on");
		//$("#bar_news2").removeClass("on");
		//$("#bar_pickup").removeClass("on");
		$("#bar_guide").parent().addClass("on");
		...
		$("#PickupBody").hide();
		$(".bar_item").removeClass("on");
		$("#bar_news").parent().addClass("on");
		$("#bar_news2").parent().addClass("on");
		//$("#bar_pickup").removeClass("on");
		$("#trendTab").removeClass("on");
		$("#trendNews").addClass("on"); 
		$("#tab_news").show(); 
		...
		$("#GuideBody").hide();
		$(".bar_item").removeClass("on");
		//$("#bar_news").removeClass("on");
		$("#bar_pickup").parent().addClass("on");
	...
	$("#bar_news").click(function(){
		...
		ga('send', 'event', 'mf_tip', 'click_tab', 'tab_today_news');
		try{
			...
			$("#PickupBody").hide();
			$(".trd_nav").hide();
			$(".tab_trend").css('margin-bottom','-5px');
		}
		ga('send', 'pageview', { 
			'page': '/mobilefirst/renew/trend_news.jsp',
			'title': 'mf_홈_오늘의뉴스' 
		}); 
	});
	...
	$("#bar_pickup").click(function(){
		...
		ga('send', 'event', 'mf_tip', 'click_tab', 'tab_today_pickup');
		try{
		...
		ga('send', 'pageview', { 
			'page': '/mobilefirst/renew/trend_news.jsp#pickup',
			'title': 'mf_홈_트렌드 픽업'  
		});  
	});
	$("#bar_news2").click(function(){
		...
		//	window.android.trend_news();
		//}catch(e){
			//location.href = "/mobilefirst/renew/trend_news.jsp#news";
			if(getCookie("verios")){
			//	location.href = "/mobilefirst/renew/trend_news.jsp?type=news";
			} 
			$(".bar_item").removeClass("on");
			$("#bar_news2").parent().addClass("on");
			//$("#bar_guide").removeClass("on");
			//$("#bar_pickup").removeClass("on");
			$("#NewsBody").show();
			$("#PickupBody").hide();
			$("#GuideBody").hide();
	...
	$("#bar_guide").click(function(){
		ga('send', 'event', 'mf_tip', 'click_tab', 'tab_guide');
		//try{
		//	window.android.trend_news();
		//}catch(e){ 
			//location.href = "#guide"; 
			if(getCookie("verios")){
			//	location.href = "/mobilefirst/renew/trend_news.jsp?type=guide";	
			}
			$(".bar_item").removeClass("on");
			//$("#bar_news").removeClass("on");
			//$("#bar_news2").removeClass("on");
...
function getShoppingGuideListTop(){
	...
	if(json_guide.top.ShoppingGuideList){
		...
		template_guide += "<ul class=\"evtbnr\">";
		
		for(var i=0;i<json_guide.top.ShoppingGuideList.length;i++){ 
			if(json_guide.top.ShoppingGuideList[i].mo_img){
				template_guide += "<li onclick=\"view_guide('"+ json_guide.top.ShoppingGuideList[i].kb_no+"', '1');\"><img src=\"http://storage.enuri.gscdn.com/pic_upload/enurinews_mobile/"+json_guide.top.ShoppingGuideList[i].mo_img+"\"  alt=\"\"></li>";
				vCnt = vCnt + 1;
			}
			if(vCnt == 4){ 
				break;
...
function CmdTimer(){
	...
	//var currentSlide = $('.evtbnr').slick('slickCurrentSlide');
	//alert(evtbnr);
	$('.evtbnr').on('afterChange', function(event, slick, currentSlide, nextSlide){
		...
		for(var i=0;i<json.trend.length;i++){ 
			if(json.trend[i].ref_model_no != undefined && json.trend[i].sublist){
				...
				if(i < 4){
					...
					template += "	<img class=\"trend_thumb\" data-original=\""+json.trend[i].ref_model_img+"\"  src=\"http://imgenuri.enuri.gscdn.com/images/mobilefirst/noimg_plan.png\" alt=\"\" style=\"width:100%;\"> ";
				}
				...
				//template += "<p class=\"trd_bnr\"><img src=\"http://imgenuri.enuri.gscdn.com/images/mobilefirst/trd_bnr.png\" alt=\"첫 구매하고 e머니받기\" /></p>";
				...
				template += "<ul class=\"relation\">	 "; 
					for(var k=0;k<json.trend[i].sublist.length;k++){
						...
						template += "		<img class=\"trend_thumb\" data-original=\""+json.trend[i].sublist[k].img_src+"\"  src=\"http://imgenuri.enuri.gscdn.com/images/mobilefirst/noimg_plan.png\" alt=\"\" /> ";
...
function displayFooter(strPg){
	if(strPg == "news" || strPg == "#news"){
		$("#footerBannerAD").show();
		$("#footerBannerBasic").hide();
	}else{
		$("#footerBannerAD").hide();
