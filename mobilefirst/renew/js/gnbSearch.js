...
jQuery(document).ready(function($) {
    //검색 입력 박스 오픈
	if(document.URL.indexOf("list.jsp") >= 0  || document.URL.indexOf("cpp.jsp") >= 0){
		getSuggestion();
	} 
    ...
    $("#search_keyword").bind({
        ...
        $("body").on('click', '.sch_input li a', function(event) {
...
function goSearch(searchText,type){
	if($("#all_delete").is(":visible")){
		 ga('send', 'event', 'mf_search', 'search', 'search_검색어_최근검색어');
	}
	var url = "/mobilefirst/search.jsp?keyword=";
	
    var keyword = ""; 
    if(type == 'A')  keyword = searchText;
    else keyword = $("#search_keyword").val();
    ...
    $.getJSON(url+searchParam(),function(data){
            ...
            //포커스 검색바 검색어 없을 경우 검색히스토리 노출
                ...
                if(varSearchType == 'recentl'){
                    ...
                    if(typeof rec_flag != "undefined" ){
                        if(rec_flag.indexOf("RECFLAG=N") > -1){
                        	$("#searchList").html("<p class=\"notxt\">최근 검색어 저장이 꺼져 있습니다.</p>");	
                        	return false;
                        }
                	}
                	
                    if($(".recent_sch .sch_tab li:first").hasClass("on")){	
                    	$("#searchList").show();
                    }else{
                    	$("#searchList").hide();
                    	searchFootDisplay("N");
                    ...
                    if (data.history){
                    	...
                    	searchWrap_visible();
                    	$("#searchList").html("<p class=\"notxt\">최근 검색어가 없습니다.</p>");	
                        //alert($(".recent_sch .sch_tab li:first").hasClass("on"));
...
function getAutoMakeHtml(){
    var makeHtml =  "<ul  class=\"sch_input\">";
        makeHtml += "{{#auto}}";
        makeHtml += "<li class={{{onoff}}} style='height:30px;'>{{{keyword}}}<em onclick=\"javascript:inSearchGo('{{{keyword}}}');\"  class=\"in_go ico_m\">검색어 입력</em></li>";
        makeHtml += "{{/auto}}";
        makeHtml += "</ul>"; 
        return makeHtml; 
}
...
function keywordReplace(objKeyword , eleKeyword){
    var returnKeyword = objKeyword.replace(eleKeyword,"<em>"+eleKeyword+"</em>"); ;
...
function historyMakeData(json){
    ...
    $.each(json.history,function(i,v){
        ...
        date= $.trim(v.keyword.substring(v.keyword.indexOf("__")+2, v.keyword.length)); 
        
        var tempJsonData = {};
        tempJsonData.history_keyword = v.keyword.trim();
        tempJsonData.keyword = $.trim(keyword);
        tempJsonData.date = date; 
...
function all_delete_rec(){
    ...
    ga('send', 'event', 'mf_search', 'search', 'search_최근검색어 전체삭제');
}
//최근검색어 삭제
function deleteRec_one(param){
    ...
    ga('send', 'event', 'mf_search', 'search', 'search_최근검색어 개별삭제');
}
function delect_rec_Y(){
    if(confirm("검색어 저장 기능을 활성화 합니다.")){
    
        CmdSetCookie('reckeyword_Y','');
        ga('send', 'event', 'mf_search', 'search', 'search_최근검색어 저장 켜기');
        setTimeout(function(){
            getSuggestion();  
            scrollTo(0,0);
        },500);        
...
function iosDelete_rec(){
   var temp = confirm('검색어 저장을 사용하지 않으시겠습니까?');
    ...
    if(temp){ 
        ...
        ga('send', 'event', 'mf_search', 'search', 'search_최근검색어 저장 끄기');
        setTimeout(function(){
            getSuggestion();  
            scrollTo(0,0);
        },1000);            
...
//최근검색어,인기검색어 탭
function CmdSchTab(param){
	if(param == "1"){
		$(".recent_sch .sch_tab li:first").addClass("on");
		$(".recent_sch .sch_tab li:last").removeClass("on");
		$(".sch_input").show();
		$(".hit_sch").hide();
		$("#sch02").hide();
		$(".notxt").show(); 
		$("#searchList").show();
		searchFootDisplay('Y');
		ga('send', 'event', 'mf_search', 'search', 'search_tab_최근검색어');
	}else if(param == "2"){
		$(".recent_sch .sch_tab li:first").removeClass("on");
		$(".recent_sch .sch_tab li:last").addClass("on");
		getSchRank();
		$(".sch_input").hide();
		$(".hit_sch").show();
		$("#sch02").show();
		$(".notxt").hide(); 
		searchFootDisplay('N'); 
		 ga('send', 'event', 'mf_search', 'search', 'search_tab_인기검색어');
	}
}

function getSchRank(){

	var loadUrl = "/lsv2016/ajax/getSearch_Keyword_Rank_ajax.jsp?procType=1&orderBy=1";		
	var makeHtml = ""; 
	...
	$.ajax({  
		...
		success: function(result){ 
			...
			if(result.keywordList){
				$.each(result.keywordList, function(i,v){
					if(i < 10){
						makeHtml += "<li onclick=\"ga('send', 'event', 'mf_search', 'search', 'search_검색어_인기검색어');goSearch('"+v.keyword+"','A');\"><em class=\"num\">"+(i+1)+"</em>"+v.keyword+"";
						if(i < 3){
							if(v.new_rank_2 > 0){
								makeHtml += "<span class=\"up\">"+v.new_rank_2+"<em>상승</em></span>";								
							}else if(v.new_rank_2 < 0){
								makeHtml += "<span class=\"down\">"+v.new_rank_2+"<em>하락</em></span>";
							}
						}
						makeHtml += "</li>";
					}
				}); 
				$("#sch02 .hit_sch").html(makeHtml);
			}
		}
	});
} 
 
function inSearchGo(param){
	$("#search_keyword").val(param.replace("<em>","").replace("</em>",""));
	$("#search_keyword").focus();
	getSuggestion();
	ga('send', 'event', 'mf_search', 'search', 'search_최근검색어 입력');
}