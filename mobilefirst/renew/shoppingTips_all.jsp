...
<div id="wrap">
	...
	<section class="news news_link">
		<div class="newsbox"> 
			<h2>쇼핑팁 전체보기<a href="#" class="ico_m">창닫기</a></h2>
			<div class="tip_scl">
				<ul class="cate_tip">
					<li id="tip_head_01" class="tip_btt" cate="01" tit="패션/뷰티">패션/뷰티</li>
					<li id="tip_head_02" class="tip_btt" cate="02" tit="유아/라이프">유아/라이프</li>
					<li id="tip_head_03" class="tip_btt" cate="03" class="on" tit="디지털/가전">디지털/가전</li>
					<li id="tip_head_04" class="tip_btt" cate="04" tit="스포츠/자동차">스포츠/자동차</li>
					<li id="tip_head_05" class="tip_btt" cate="05" tit="컴퓨터">컴퓨터</li>					
				</ul>
			</div>
...
var json_base = (function() {
    ...
    $.ajax({
        'async': false,
        'global': false,
        'url': "/mobilefirst/api/main/shoptipTab_all.jsp",
        'dataType': "json",
...
$(function() {
    ...
    if(vTip_no_now=="03"){
    	$(".tip_scl").scrollLeft(100);
    }
    else if(vTip_no_now=="04" || vTip_no_now=="05"){
    	$(".tip_scl").scrollLeft(300);
    }
	$(".cate_tip > li").click(function(){
		...
		$(window).scrollTop(0);
		fnGa('mf_쇼핑팁','click_전체보기','전체보기_'+vTitle);
	});
	...
	$(".ico_m").click(function(){
		fnGa('mf_쇼핑팁','click_전체보기','전체보기_닫기');
		location.href="/mobilefirst/index.jsp#4";
	});
        ...
        for(var i = 0;i<json_list.length;i++){
			...
			if(vList.length > 0){
				for(var j=0;j<vList.length;j++){
					...
					template += "<li class=\"tiplist tip_"+ vTip_no +"\" onclick=\"fnGa('mf_쇼핑팁','click_전체보기','전체보기_"+ vCategory_name+"_"+ vList[j].kbno +"');show_detail('"+ vList[j].url +"');\">";
					if(vTmpImg != ""){
						if(j<10){
							template += "<span class=\"thum\"><img class=\"lazy\" src=\""+ vTmpImg +"\" data-original=\"\" alt=\"\" /></span>";						
						}else{
					...
					template += "	조회수 "+ commaNum(vList[j].kb_readcnt);
					if(vList[j].source != ""){
