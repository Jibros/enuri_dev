...
function getEDPlatformData(){
	...
	var intMaxCount = 4;											// 필요한 광고 갯수. 광고슬롯갯수만큼만 호출	
...
//	var strUa = "";														// 사용자 HTTP 헤더의 User-Agent값 - 필수 아님. (UTF-8 Encoding 필수)
	...
	**********************************/
	...
	if (varEDKeyword.length > 0) {
	 ...
	 /*if (varEDSearchPage.indexOf("list.jsp") > -1) {   
		 strChannelId = "E001004001";
	 }else  if (varEDSearchPage.indexOf("search.jsp") > -1) {   
	...
	*/
	...
	if (varEdIsReal) {    
		varEdReqUrl = varEdReqUrl + "http://display.cpcad.enuri.com/adReq?";	//운영
	} else {  
...
function mobileListHtmlRandering (data, strQuery, strAdType) {
	...
	for (var i=0; i<data.adResultItems.length; i++) {
		...
		if (varEdIsReal) {
			randerUrl = "http://www.enuri.com/mobilefirst/move.jsp?sb=F&plno="; //운영 : sb=F 로딩 이미지 안 보이도록 함.
		} else { 
			randerUrl = "http://dev.enuri.com/mobilefirst/move.jsp?sb=F&plno="; //데브 : sb=F 로딩 이미지 안 보이도록 함.
		}  
		...
		//직접 등록 기능 추가 - 직접 등록은 plno가 없는 관계로 다르게 랜더링 해야 함.
		if (plno > 0) {
			objItemId
			.find(".best_mall img").attr("src", shopImageUrl).end();
		} else {
			objItemId
			.find(".price span").html("에누리판매가").end()		//에누리판매가로 텍스트 변경
			.find(".best_mall").html(shopNm).end();			//Mall 이미지 대신 text 형태로 삽입
		}
		...
		//$('.prodList li:eq('+(i+6)+')').after(objItemId);
		$('.prodList #adline').before(objItemId);
		edplatfromHTML +="<li id=\"plusLinkGoodsItemCron_"+(i+1)+"\" class=\"pluslink_goods_list\" onclick=\"window.open('"+randerUrl+"', '_blank');\">"+ objItemId.html()+"</li>";
