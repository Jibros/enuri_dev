	...
	if(listType==2) {
		...
		$.ajax({
			...
			success : function(ajaxResult) {
				...
				listGoodsCnt = $(".lp_list.grid > ul > li").size();
				...
				// 찜하기 버튼 체크
				if($(".lp_list.grid > ul > li").size() > 0) {
					//zzimCheckSet();
					// 총 개수를 세팅
					$(".listTotalNum .showCnt").html(listGoodsCnt+"개 상품");

				}else{
					$("#container .no_txt").html("폴더에 찜한 상품이 없습니다.");
					loadBodyItems(folder_id, loadPageNum);
					$("#container .no_txt").css("display", "block");
				}
				...
				pagingListNextObj.off('click').on('click', function (e) {
					...
					if($("#container .no_txt").is(':visible')){
					}else{
...
//				} catch(e) {}
		...
		if(listType==1) {
			...
			if($(".lp_list.grid > ul > li").size() > 0) {
				zzimCheckSet();
				// 총 개수를 세팅
				$(".listTotalNum .allCnt").html(listGoodsCnt+"개 상품");
			}else{
				loadBodyItems(folder_id, loadPageNum);
			}
		}
...
function loadBodyItems(tempFolder_id, sPageNum) {
	...
	var nonTxtObj = $(".no_txt");
	nonTxtObj.css("display", "none");

	var prodListUlObj = $("#container .lp_list.grid ul");
	//prodListUlObj
	...
	$.ajax({
		...
		//	console.log("listGoodsCnt>>>"+listGoodsCnt);
		//	console.log("listGoodsAllCnt>>>"+listGoodsAllCnt);

			if(listGoodsAllCnt > listGoodsCnt && listType==1) {
				listGoodsCnt = listGoodsAllCnt;
			}

		//	console.log("listGoodsCnt2>>>"+listGoodsCnt);
	...
	if(listType==2) {
		...
		$.getJSON(loadUrl, null, function(data) {
			...
			$.each(folderJsonObj, function(indexI, folderListObj) {
				...
				tempFolder_cnt = folderListObj["folder_cnt"];
...
function setFolderUI() {
	...
	for(var i=0; i<folderShowFlag.length; i++) {
		if(folderShowFlag[i]==1) {
			...
			zzimFolderSelObj.append("<option value=\""+(i+1)+"\" "+checkedStr+">찜폴더"+(i+1)+"&nbsp("+folderItemsCnt[i]+")</option>");
