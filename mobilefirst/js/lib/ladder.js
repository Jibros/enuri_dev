...
function ladderjs(){
    ...
    var userName = ""; // 클릭된 node split ex) 1~4
...
/*	$(document).on('click', '.start_ladder', function(e){
		...
		if(!userName){// 다짐 선택 안할 경우 alert
			alert("다짐을 선택하세요");
		}else{
			$(this).animate({'opacity': 0, 'top': 60}, 300, 'linear', function(){
	...
	//수정
	this.ladder_start_click = function(){
		//console.log(userName);
		if(!userName){// 다짐 선택 안할 경우 alert
			alert("다짐을 선택하세요");
		}else{
			...
			$("#clickInsert").animate({'opacity': 0, 'top': 60}, 300, 'linear', function(){
				startLineDrawing(userSelectNode, userColor);
			});
    ...
    function resultSetting(){
        ...
        for(var i=0; i <  resultList.length; i++){
			...
			// 문화상품권 class="prizes1" 추가
			// 국대떡볶이 class="prizes2" 추가
			// 100 e머니 class="prizes3" 추가
			// 던킨 class="prizes4" 추가
