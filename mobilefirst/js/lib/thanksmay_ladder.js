$(function(){
	...
	$(document).on('click', '.start_ladder', function(e){
		var result = 1;
		$('.start_ladder').animate({'opacity': 0, 'top': 60}, 300, 'linear', function(){
			startLineDrawing(userSelectNode, userColor);
		});
		$(document).off('click', 'button.ladder-start');
		setTimeout(function(){
    		$(".answer-wrap.open").removeClass("open").addClass("prizes"+result);
    		$('#prizes').show();
    		
    		var prizes = $(".answer-wrap");
    		var n = [1,2,3,4];		//배열 선언       	
        	for(var j, x, i = n.length; i; j = Math.floor(Math.random() * i), x = n[--i], n[i] = n[j], n[j] = x);
        	n.splice(n.indexOf(Number(result)),1);
        	
        	var x = 0;				//변수 x 선언, 0으로 초기화 
    		for(var i=0;i<prizes.length;i++){							// prizes의 갯수만큼 반복
    			if(!$(prizes[i]).hasClass("prizes"+ result)){	// answer-wrap prizes+결과값 이 아니면, 즉 당첨된게 아니면
    				$(prizes[i]).addClass("prizes"+n[x]);				// 클래스를 추가한다 prizes0
    				x++;												//1
    			}
    		}
    	},2100);    
        ...
        for(var y=0; y < heightNode; y++){
            for(var x =0; x <widthNode; x++){
                ...
                if(nodeInfo["change"] && nodeInfo["draw"] ){
                     stokeLine(x, y ,'w' , 'r' , '#fff' , borderThick)
                }else{
    ...
    function stokeLine(x, y, flag , dir , color , width){
        ...
        ctx.moveTo(moveToStart +3, moveToEnd  + 6);
        ctx.lineTo(lineToStart +3, lineToEnd  + 6);
        ...
        html += '<table>'
         for(var y =0; y < heightNode; y++){
            ...
            for(var x =0; x <widthNode-1 ; x++){
                html += '<td style="width:' + widthNum + 'px; height:' + heightNum + 'px; border-left:'+borderThick+'px solid #fff; border-right:'+borderThick+'px solid #fff;"></td>';
            }
            html += '</tr>';
        }
        html += '</table>'
