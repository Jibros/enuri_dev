...
<%!public static final int MINIMUM_GOODS_SIZE = 10;
	...
	public String getApiUrl(String msg) {
		...
		String arn = "";
		String title = "[웹1팀 노병원] 종합몰 크롤링 메시지";
...
/*
	...
	try {
		...
		System.out.println("==========onCrawlingCJ=========");
...
public String onCrawlingCJ(){
	try {
		...
		String[] urlList = new String[]{
				"http://display.cjmall.com/c/rest/category/getLLCateTop100ItemList?llCateId=G00001&cjEmpYn=false&type=P",
				"http://display.cjmall.com/c/rest/category/getLLCateTop100ItemList?llCateId=G00002&cjEmpYn=false&type=P",
				"http://display.cjmall.com/c/rest/category/getLLCateTop100ItemList?llCateId=G00003&cjEmpYn=false&type=P",
				"http://display.cjmall.com/c/rest/category/getLLCateTop100ItemList?llCateId=G00004&cjEmpYn=false&type=P",
				"http://display.cjmall.com/c/rest/category/getLLCateTop100ItemList?llCateId=G00005&cjEmpYn=false&type=P",
				"http://display.cjmall.com/c/rest/category/getLLCateTop100ItemList?llCateId=G00006&cjEmpYn=false&type=P",
				//"http://display.cjmall.com/c/rest/category/getLLCateTop100ItemList?llCateId=G00007&cjEmpYn=false&type=P",
				//"http://display.cjmall.com/c/rest/category/getLLCateTop100ItemList?llCateId=G00008&cjEmpYn=false&type=P",
				//"http://display.cjmall.com/c/rest/category/getLLCateTop100ItemList?llCateId=G00009&cjEmpYn=false&type=P",
				//"http://display.cjmall.com/c/rest/category/getLLCateTop100ItemList?llCateId=G00010&cjEmpYn=false&type=P",
				//"http://display.cjmall.com/c/rest/category/getLLCateTop100ItemList?llCateId=G02363&cjEmpYn=false&type=P"
				};
		...
		for(String url : urlList) {
			...
			JSONArray rootJson = jsonObjectUrlParsing(url).getJSONObject("result").getJSONArray("cateTop100ItemTupleList");
			...
			for(int i=0; i<rootJson.length() && !bbb; i++) {
				...
				JSONObject jsonGoods = rootJson.getJSONObject(i);
				String itemCode = jsonGoods.getString("itemCd");
				
				System.out.println("itemCode :"+itemCode);
				
				itemCodeList.add(itemCode);
				//if(itemCodeList.size() > 990)	break;
			}
		}
		...
		if(result.getBoolean("result")) {
			JSONObject result2 = socialProc.makeGoodsDataJSON(shopCode, "CJ온스타일");
			checkCrawlingError(result2, "CJ온스타일");
			return result2.getString("result");
		} else {
			//checkCrawlingError(result, "CJmall");
			return result.getString("result");
	...
	public String onCrawlingSSG() {
		try {
			int shopCode = 47;
			String[] shopName = new String[] { "ssg"};
			Mobile_Social_GoodsToPrice_Proc socialProc = new Mobile_Social_GoodsToPrice_Proc();
	...
	public String onCrawlingEmart() {
		try {
			int shopCode = 374;
			String[] shopName = new String[] {  "emart" };
			Mobile_Social_GoodsToPrice_Proc socialProc = new Mobile_Social_GoodsToPrice_Proc();
			//JSONObject result = socialProc.insertSsgGoodsData();
			...
			if (result.getBoolean("result")) {
				...
				checkCrawlingError(result2, "emart");
				return result2.getString("result");
			} else {
				checkCrawlingError(result, "emart");
				return result.getString("result");
			}

		} catch (Exception e) {
			callEmsAPI(getApiUrl("try/catch [emart] " + e.toString()));
			return e.toString();
		}
	}
	...
	try {
		...
		*/
		...
		try {
			int shopCode = 57;
			String[] shopName = new String[] {  "hmall" };
			Mobile_Social_GoodsToPrice_Proc socialProc = new Mobile_Social_GoodsToPrice_Proc();
			//JSONObject result = socialProc.insertSsgGoodsData();
