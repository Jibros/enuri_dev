...
<%
	...
	String LOCAL_URL = "100.100.100.80:8080" + "/mobilefirst";
	...
	String[][] pagesdev = { 
			{"http_array", BASE_URL + "/http/json/mobile_deal_list.json" },
			{"http_array", "http://dev.enuri.com/mobilefirst/http/social/mainBanner.json" },
			{"http_array", BASE_URL + "/http/json/shopMainLogo.json" }
		};
	...
	try {
		...
		makeJson.put("serverTarget", SERVER_TARGET);
		for(int i=0; i<pagesdev.length; i++) {
			String[] temp = pagesdev[i][1].split("/");
			String fileName = temp[temp.length-1]; 
			String type = pagesdev[i][0];
			
			if(type.equals("http_array")){
				JSONArray jsonObj = getJSONArrayfromHttp(pagesdev[i][1]);
				makeJson.put(fileName, jsonObj);
			} else if (type.equals("http_object")){
				JSONObject jsonObj = getJSONObjectfromHttp(pagesdev[i][1]);
				makeJson.put(fileName, jsonObj);
			}
		}
		...
		*/
		...
		{	
			...
			*/
			try {
				String hotClickUrl = "http://"+LOCAL_URL + "/api/deal/hotClick_HomeItem.jsp";
		...
		*/
		{
			...
			*/
			try {
				makeJson.put("today_base_data", (JSONArray)((JSONObject)getJSONObjectfromHttp(BASE_URL + "/api/deal/realTimeDeal.json").get("list")).get("DS_COUPON"));
