...
if(bestType.equals("2")) {
	...
	url = "http://mobile.gmarket.co.kr/Best/Category";
	param = "code=G00";
	encodingStr = "utf-8";
	type = 2;
	shop_code = 536;
...
if(bestType.equals("3")) {
	...
	url = "http://m.11st.co.kr/MW/CMS/PageDataAjax.tmall?pageId=MOHOMEBESTV3";
	param = "";
	encodingStr = "euc-kr";
	type = 1;
	shop_code = 5910;
	...
	if(bestType.equals("3")) {
				...
				if(listItem!=null) {
					if(listItem.has("prdNo")) {
						...
						if(listItem.has("prdImgUrl")) {
							...
							System.out.println("=====11111========"+listItem.getString("prdNo"));
		...
		*/
		...
		for(int i=0; i<topList.length(); i++) {
			...
			JSONArray jsonArr =  commonItem.getJSONArray("blockList");
			for( int j=0; j<jsonArr.length(); j++ ){
				
				String type_St = jsonArr.getJSONObject(j).getString("type");
				if("ProductGrid_Ranking_Review".equals(type_St)){
					
					JSONArray list = jsonArr.getJSONObject(j).getJSONArray("list");
					
					for( int k=0; k < list.length(); k++ ){
						String prdNo = list.getJSONObject(k).getString("prdNo");
						
						if(goodsCodeList.length()>0) goodsCodeList += ",";
						goodsCodeList += prdNo;
						
						String imageUrl1 = list.getJSONObject(k).getString("imageUrl1");
						imageUrlHash.put(prdNo,imageUrl1 );	
					}
...
if(goodsCodeList.length()>0) {
		...
		for(int i=0; i<goodsCodeListAry.length; i++) {
			if(goodsCodeListAry[i].length()>0) {
				...
				if(mgtpd!=null) {
					...
					//센딜
					if(bestType.equals("5")) jSONObject.put("gd_kn_cd","H"); //핫딜
					else 					 jSONObject.put("gd_kn_cd","B"); //베스트
	...
	out.println("}");
	//mobile_goodstopricelist_proc.hotDealInsUpd(jSONArray);
}
%>
<%!
public JSONObject jsonUrlParsing(String strUrl) throws UnsupportedEncodingException, MalformedURLException, IOException{
    ...
    if(sbJson != null){
        try {
            ...
            System.out.println("********* shopBestMake jsonUrlParsing **************:"+strUrl+"======"+e);
        }
    }
    return null;
}
