...
// 					jSONObject.put("option",""+option+"");
	...
	if(bestType.equals("3")) {
		try {
			...
			String[] typeList = {"FRESH", "PROCESS", "DRINK", "BABY", "LIFE"};
			int[][] codeList = {{8930243,8930246,8930248,8930249,8930251,8930254,8930261,8930262,8930263},
								{9294721,9294723,9294733,9294734,9294735,9294750,9294760,9294761,9294762},
								{9294793,9294795,9294796,9294797,9294818,9294819,9294825,9294826,9294828},
								{9294867,9294868,9294869,9294870,9294871,9294872,9294873,9294909,9294910},
								{9294911,9294912,9294915,9294922,9294930,9294931,9294933,9294934,9294935}};

			ArrayList<HashMap> al = new ArrayList<HashMap>();  
			
			for(int i = 0; i < typeList.length; i++){
				for(int j = 0; j < 9; j++){
		            InputStream input = new URL("http://m.11st.co.kr/MW/Mart/getFastCartPrdList.tmall?listType=PRD&dispObjNo="+codeList[i][j]+"&spceType="+typeList[i]+"&mallType=mart").openStream();
		            Document docFirst = Jsoup.parse(input, "euc-kr", "http://m.11st.co.kr/MW/Mart/getFastCartPrdList.tmall?listType=PRD&dispObjNo="+codeList[i][j]+"&spceType="+typeList[i]+"&mallType=mart");
	...
	*/
	if(bestType.equals("3")) {
		try {
			...
			for(int s = 0; s < subCtgrNo.length; s++){
				int k =0;
				Document doc = Jsoup.connect("http://www.11st.co.kr/browsing/MainThemeAction.tmall?method=listingData&ctgrNo=152714&subCtgrNo="+subCtgrNo[s]+"&dispCtgrNo="+dispCtgrNo[s]).get();
				Elements divLink = doc.select(".product_type1");
				for (Element eleDiv : divLink) {
					Elements eleItem = eleDiv.select(".add a");
					String item = eleItem.attr("href");
					item = StringUtils.substringAfter(item,"prdNo/");		
					arrlist1.add(item); 
					 
					Elements imgUrl =  eleDiv.select(".thumb img"); 
					String imgSrc = imgUrl.attr("src");
					arrlist2.add(imgSrc);
					
					String orgPrice = "";
					arrlist3.add(orgPrice);
					if(k == 19){
						break;
					}
					k++; 
				...
				int totalCnt = 0;
				for(int i=0 ; i < arrlist1.size(); i++){
					String item = arrlist1.get(i).toString();
					item = item.replace("[", "").replace("]", "");
					String strImg = arrlist2.get(i).toString();
					strImg = strImg.replace("[", "").replace("]", "");
					String orgPrice = arrlist3.get(i).toString();
