...
public void alertTak(String userid, String type){
	...
	*************************************************************/
	...
	try{
		...
		if( "24".equals(type) || "26".equals(type)){
			msg.append( "[에누리 가격비교]\n");
			msg.append( "초대 혜택 이벤트 적립 완료\n\n");

			msg.append( "안녕하세요. "+userNm+"님!\n");
			msg.append( "초대 혜택 이벤트에 참여해 주셔서 감사합니다.\n\n");

			msg.append( "초대 혜택 e9,000점이 적립되었습니다!\n\n");
		...
		if (receiver_num.length() > 10) {
			...
			if (rtnArr == null || rtnArr.length() <= 0) {
				logMsg("new_frend_kakao_send_msg.jsp", "### send post return error... data=>"+bizData.getJson());
			} else {
				for (int j=0; j<rtnArr.length(); j++) {
					if (!bizProc.updRtnMsg(rtnArr.getJSONObject(j))) {	// status Ready > Save
						logMsg("new_frend_kakao_send_msg.jsp", "### update fail... msgid:" + bizData.getMsgid() + ", rtnArr:"+rtnArr.toString());
					}
				}
			}
		}
...
private void logMsg(String method, String msg) {
	System.out.println("["+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(Calendar.getInstance().getTime())+"] === /mobilefirst/bizmsg/new_frend_kakao_send_msg.jsp === " + method + "=> " + msg);
}
%>