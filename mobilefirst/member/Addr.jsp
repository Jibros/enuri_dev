...
<%
	...
	if(!strKeyword.equals("")){
		...
		if(strNewType.equals("Y")){ //도로명 주소
			...
			if(strKeyword.indexOf(" ")>0){
				road_name = strKeyword.substring(0, strKeyword.indexOf(" "));		//첫번째인자값
				bd_code1 = strKeyword.substring(strKeyword.indexOf(" ")+1).trim();	//두번째 인자값

				if(bd_code1.indexOf(" ")>0) {	//세번째 인자값이 있을 경우
					bd_code2 = bd_code1.substring(bd_code1.indexOf(" "));		//세번째 인자값 저장
					bd_code1 = bd_code1.substring(0, bd_code1.indexOf(" "));	//세번째 인자값 제거 후 두번째 인자값 재저장

					//두번째 인자값이 숫자가 아닌 경우 첫번째 인자값과 합하고
					//세번째 인자값을 두번째 인자값으로 교체
					if (!Validation.isNum(bd_code1)) {	// 두번째 인자값이 숫자가 아닌 경우
						road_name = road_name + bd_code1;
						bd_code1 = bd_code2.trim();

						//-가 있을 경우 " " 로 치환하고
						bd_code1 = bd_code1.replaceAll("-", " ");
						if (bd_code1.indexOf(" ") > 0) {	//-인 경우 확인
							bd_code2 = bd_code1.substring(bd_code1.indexOf(" ")).trim();		//세번째 인자값 저장
							bd_code1 = bd_code1.substring(0, bd_code1.indexOf(" ")).trim();	//세번째 인자값 제거 후 두번째 인자값 재저장
						}
						if (!Validation.isNum(bd_code2)) {	// 세번째 인자값이 숫자가 아닌 경우 초기화 (검색 범위를 늘린다.)
							bd_code2 = "";
						}
					} else {	//두번째 인자값이 숫자인 경우
						if (!Validation.isNum(bd_code2)) {	// 세번째 인자값이 숫자가 아닌 경우 (검색 범위를 늘린다.)
							bd_code2 = "";
						}
					}
				} else {		//세번째 인자값이 없는 경우
			...
			//한글 체크 로직 : - 또는 " " 외에 다른 한글이 들어갔을 경우 리플레이스 처리
			String txt = null;
			char[] txtChar = null;
			txt = bd_code1;
