...
<div id="wrap">
	<div class="Maincontent">
		<!--<p class="cpp_bnr"><img src="http://imgenuri.enuri.gscdn.com/images/m_home/@cpp_bnr.png" alt=""  /></p>-->
...
jQuery(document).ready(function($) { 
	...
	function getCategory(){
		...
		var loadUrl = "/mobilefirst/gnb/category/cppCategory.json";
		var img_count = 0;
		var class_on ="";
		$.ajax({
			url: loadUrl,
			...
			success: function(json){ 
	                ...
	                if(v.g_parent == gcate){
	                	if(img_count==0){
	                		html += "<li class=\"on\" id=\"cpp_"+v.g_parent+"_"+v.g_cate+"\" onclick=\"ga_log_cate_top('"+v.g_name+"');\"><img src=\"http://imgenuri.enuri.gscdn.com/images/m_home/cpp_"+v.g_parent+"_"+v.g_cate+"_on.png\" /><span class=\"dp\" >"+v.g_name+"</span></li>";
	                		class_on = v.g_seqno;
	                	}else{
	                		html += "<li id=\"cpp_"+v.g_parent+"_"+v.g_cate+"\" onclick=\"ga_log_cate_top('"+v.g_name+"');\"><img src=\"http://imgenuri.enuri.gscdn.com/images/m_home/cpp_"+v.g_parent+"_"+v.g_cate+".png\" /><span class=\"dp\">"+v.g_name+"</span></li>";
	                	}
				...
				if(img_count<5){
					var adminHtml = "";
               		var adminLoadUrl = "/mobilefirst/http/json/cppAppJson1.json";
 	                	$.ajax({
 	                		url: adminLoadUrl,
 	          				dataType: 'json',
 	          				async: true,
 	          				success: function(json){ 
 	          					$.each(json.cppHeadCate.cpp_list, function(i, v) {
			...
			//ga_log_cate_top(change_on);
				...
				if(change_on !=''){
					 $(".maincate li").each(function () {
						 ...
						 //이미지 off
						 if(change_off != ''){
							 $("#"+change_off+" img").attr("src","http://imgenuri.enuri.gscdn.com/images/m_home/"+ change_off +".png");
							 //클래스 off
							 $("#"+change_off).removeClass("on");
						 }
					 });
					 //선택된 이미지 on
					 $("#"+change_on+" img").attr("src","http://imgenuri.enuri.gscdn.com/images/m_home/"+ change_on +"_on.png");
					 //선택된 이미지 off 
	...
	function getNews(){
		...
		var loadUrl = "/mobilefirst/ajax/cppAjax/getNews.jsp?gcate="+gcate;
		
		if(gcate > 4){
			loadUrl = "/mobilefirst/ajax/listAjax/getNews2.jsp?gcate="+gcate;
		}
		...
		$.ajax({
			...
			success: function(json){
				...
				$.each(json.NewsList , function(i, v){
	                ...
	                if( i < 2 && gcate <= 4){ 
	                	...
	                	html_img += "	<img src=\"http://storage.enuri.gscdn.com/pic_upload/main/news/"+v.mn_img+"\" alt=\"\" class=\"thumb\" />";
	...
	function getBanner(){
		...
		var loadUrl = "/mobilenew/gnb/category/mainBanner.json";
        var vHtml = ""; 
	...
	function getShoppingGuide(){
		...
		var loadUrl = "/mobilefirst/ajax/cppAjax/getShoppingGuide.jsp?gcate="+gcate;
		...
		$.ajax({
			...
			success: function(json){
				...
				$("#shoppingGuideList").append(Mustache.render($shoppingGuide.html(), json));// 템플릿에 연동
				//$("img.lazy").lazyload();
	...
	function getPlan(){
		...
		var loadUrl = "/mobilefirst/ajax/cppAjax/getPlan.jsp?gcate="+gcate;
		...
		$.ajax({
			...
			success: function(json){
				...
				$("#PlanList").append(Mustache.render($Plan.html(), json));// 템플릿에 연동
				//$("img.lazy").lazyload();
				$('#PlanList').slick({
					  dots: true,
					  infinite: true,
	...
	function getSocial(){
		...
		var loadUrl = "/mobilefirst/ajax/cppAjax/getSocial.jsp?gcate="+gcate;
		...
		$.ajax({
			...
			success: function(json){
				...
				$("#socialList").append(Mustache.render($social.html(), json));// 템플릿에 연동
				//$("img.lazy").lazyload();
			}
		});
	}
	...
	function getMainList(){
		...
		var loadUrl = "/mobilefirst/http/json/cppJson"+gcate+".json";
			...
			success: function(json){ 
				...
				$.each(items, function(i, v){
	                ...
	                if(i < 50){
						...
						vHtml += "<span class=\"mall\" id=\"mall_"+v.modelno+"\" ><img src=\"http://imgenuri.enuri.gscdn.com/images/view/ls_logo/2013/Ap_logo_"+v.shopcode+".png\" alt=\""+v.shopname+"\" onerror=\"this.style.display='none';CmderrLogo("+v.modelno+",'"+v.shopname+"');\"  /></span> ";
...
function goSocial(param){
	CmdGA("gcate<%=strGcate%>_social", param);
	window.open(param);
...
function CmdErrImg(param, img){
	$("#img_"+param).error(function(){
		$(this).attr("src","http://imgenuri.enuri.gscdn.com/images/mobilefirst/noimg_plan.png");
	}).attr("src", img);
} 
...
{{#planList}}
	<li onclick="goPlan('{{{TODAY_ID}}}')"">
		<img src="http://img.enuri.gscdn.com/images/mobilefirst/planlist/B_{{{TODAY_ID}}}/B_{{{TODAY_ID}}}_main.png" alt="" />
	</li> 
{{/planList}}
</script>
<!--쇼핑가이드 템플릿-->
...
{{#ShoppingGuideList}}
	...
	<img src="http://storage.enuri.gscdn.com/pic_upload/enurinews_mobile/{{{mo_img2}}}" alt="" class="thumb" />
...
{{#goods}}
	<li>
		<a href="javascript:goSocial('{{{url}}}')">
			<img src="{{{img2}}}" class="thum" />
			<span class="mall"><img src="http://imgenuri.enuri.gscdn.com/images/view/ls_logo/2013/Ap_logo_{{{company}}}.png"
			onerror="if (this.src != 'http://imgenuri.enuri.gscdn.com/images/board/big/logo_{{{company}}}b.gif') this.src = 'http://imgenuri.enuri.gscdn.com/images/board/big/logo_{{{company}}}b.gif';"				 
			</span>
			<strong>{{{name}}}</strong>
