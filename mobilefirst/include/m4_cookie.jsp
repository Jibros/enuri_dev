...
<%
	...
	if(strType.equals("today") || strType.equals("today_d")){
		...
		cb.responseAddCookie(response);		
	}else if(strType.equals("recentl_one")){ 		//최근검색어 삭제_한개
		String stKeywordCookie = cb.GetCookie("MOB_SEAR_HISTORY","SEARCHKEYWORD");
		System.out.println(stKeywordCookie);
		System.out.println(strKeyword);
		stKeywordCookie = stKeywordCookie.replace(strKeyword,"");
		stKeywordCookie = stKeywordCookie.replace(",,",",");
		...
		cb.SetCookie("MOB_SEAR_HISTORY", "SEARCHKEYWORD", stKeywordCookie);
		cb.SetCookieExpire("MOB_SEAR_HISTORY", 3600*24*30);
		cb.responseAddCookie(response);	
	}else if(strType.equals("reckeyword_N")){	//검색어저장 안함
		cb.SetCookie("MOB_REC_FLAG", "RECFLAG", "N");
		cb.SetCookieExpire("MOB_REC_FLAG", 3600*24*30);
		cb.responseAddCookie(response);		
	}else if(strType.equals("reckeyword_Y")){	//검색어저장 안함-해제
		cb.SetCookie("MOB_REC_FLAG", "RECFLAG", "");
		cb.SetCookieExpire("MOB_REC_FLAG", 3600*24*30);
		cb.responseAddCookie(response);		
	}else if(strType.equals("m_recentl_one")){
	
		String stKeywordCookie = cb.GetCookie("MOB_SEAR_HISTORY","SEARCHKEYWORD");
		
		//stKeywordCookie = stKeywordCookie.replace(strKeyword,"");
		
	//	System.out.println("stKeywordCookie:"+StringUtils.trim(stKeywordCookie));
	...
	//	System.out.println("=========="+StringUtils.trim(stKeywordCookie)+"==========");
		...
		if(stKeywordCookie.length() > 0){
			//stKeywordCookie = StringUtils.substringAfterLast(stKeywordCookie,"");
		}
