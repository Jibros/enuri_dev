		...
		if(intNowGoodsGroupDataLength>0) {
			for(int j=0; j<intNowGoodsGroupDataLength; j++) {
				...
				*/
				...
				String strLandingUrl = "http://www.enuri.com/detail.jsp?modelno="+String.valueOf(intMutilModelNo);
				...
				if(intMutilModelNo>0 && vshopCode>0){
					pricelist_data = Pricelist_Proc.getShopMinGoods(intMutilModelNo, vshopCode);
				}
				if(pricelist_data!=null){
					shopMinPrice = pricelist_data.getPrice();
					shopURL = pricelist_data.getUrl();
				}
				if(shopMinPrice>0){
					objGroupData.put("shopGoodsYN","Y");
					objGroupData.put("shopGoodsURL",shopURL);
				}else{
					objGroupData.put("shopGoodsYN","N");
					objGroupData.put("shopGoodsURL","");
				}
				
				if(strGoodsGroupMinPrice.length()>0){
					if(Integer.parseInt(strGoodsGroupMinPrice.replaceAll("\\,", ""))==shopMinPrice){
						objGroupData.put("shopGoodsMinprice","Y");
					}else{
						objGroupData.put("shopGoodsMinprice","N");
					}
				}
