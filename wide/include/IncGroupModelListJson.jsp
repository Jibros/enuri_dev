	...
	for(int i=0;i<goods_data.length;i++) {
		...
		if(intNowGoodsGroupDataLength>0) {
			for(int j=0; j<intNowGoodsGroupDataLength; j++) {
				...
				if(strIsDelivery.equals("Y") || !strDevice.equals("pc")) {
				...
				*/
				...
				// 단위당 환산가 정보
				if(j==0) {
					groupDataMap.put("unit", unit);
					groupDataMap.put("change", String.valueOf(change));
					...
					if(CutStr.cutStr(goods_data[i].getCa_code(),6).equals("163604") && standard > 0 && unitPriceTxt > 0) {
						...
						groupDataMap.put("unitPriceMsg", unitPriceMsg);
					}
				}
				
				objGroupData.put("strUnitPrice",formatter.format(unitPrice));
				...
				objGroupData.put("strPrice",strGoodsGroupMinPrice);
				...
				objGroupData.put("strCondiName",toJS2(strBeginnerDicCondiname));
				...
				objGroupData.put("strMallCnt",strMallCnt);
