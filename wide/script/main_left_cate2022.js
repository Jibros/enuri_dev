var isDev = (top.location.href.indexOf("dev.enuri.com") > -1 
			|| top.location.href.indexOf("stagedev.enuri.com") > -1
...
$(function(){ // 메인 > 카테고리 퍼블 테스트용
	/*
	 $('body').on('mouseenter','.cate-item--depth1', function(){
         $('.cate-item--depth3, .cate-item--depth4, .cate-item--depth5').removeClass('active');
         $('.expand_left').css('padding-right', '0');
     });
     */
     var dist = 257;
     $('body').on('mouseenter','.cate--common_trigger > li > div, .cate--common_trigger > li > a', function(e){
         // depth5에서 4로 넘어올때 계속 중복입력이되서..변경함..
         // 혹시나 사용시 classList[0]확인;  해당클래스 값이 반드시 먼저 기재되어야함. 
         var targetParent = e.currentTarget.parentNode;
         var targetNext = e.currentTarget.nextSibling;
         var targetParentClass = targetParent.classList[0];

         if(targetParentClass == 'cate-item--depth3') {
             $('.cate-item--depth3').removeClass('active');
             if(targetNext != null) $('.expand_left').css('padding-right', dist);
             else $('.expand_left').css('padding-right', '0');
         }
         if(targetParentClass == 'cate-item--depth4') {
             $('.cate-item--depth5').removeClass('active');
             if(targetNext != null) $('.expand_left').css('padding-right', dist*2);
             else $('.expand_left').css('padding-right', dist);
         }
         if(targetParentClass == 'cate-item--depth5') {
             if(targetNext != null) $('.expand_left').css('padding-right', dist*2);
             else $('.expand_left').css('padding-right', dist*2);
         }
        ...
        "mouseenter" : function(){
            ...
            delayTimer = setTimeout(function(){
                ...
                $('.expand_left').css('padding-right', '0');
                    ...
                    var depth = $t.find("ul.cate--depth2 > li").length;
                    
                    if( depth == 0){
                    	var getUrl = "";

                    	if(isDev)	getUrl = "/wide/main/ajax/Ajax_Gnb_Src_2022_main.jsp?g_seq="+(inx+1);
                    	//if(isDev)	getUrl = "/wide/main/ajax/Ajax_Gnb_Src_2021_main_nw.jsp?g_seq="+(inx+1);
    ...
    $("body").on('mouseenter', '.cate-item--depth3' , function(e) {
            var $t = $(this);
            delayTimer = setTimeout(function(){
            	$t.parent().parent().parent().find("li.cate-item--depth3").removeClass("active");
                $t.addClass("active");
            },delayTime);
...
function allCate(){
	...
	if($.trim(jQuery("#all_cate").html())==""){
		jQuery.ajax({
			url : isDev ? "/wide/main/ajax/Ajax_Gnb_Src_2022_main.jsp?stype=allCate" : "/wide/main/ajax/AllCateList_2021.html",
			data : "html",
			success:function(data){
