(function (global, factory) {
    ...
    var prodPriceCompView = exports.prodPriceCompView = function prodPriceCompView(json) {
        if (json.success && json.total > 0) {
            ...
            $("#prod_pricecomp").find(".comparison__cont .head__opt .toggle__chk").unbind().click(function () {
                var toggleObj = $("#prod_pricecomp").find(".comparison__cont .head__opt .toggle__chk");
    ...
    var prodPriceCompShopListView = function prodPriceCompShopListView(json) {
        ...
        if (krShopList.length > 0) {
            ...
            $.each(krShopList, function (index, listData) {
        ...
        if (ovsShopList.length > 0) {
            ...
            $.each(ovsShopList, function (index, listData) {
        ...
        $("#kr_shoplist .btn__sel, #ovs_shoplist .btn__sel").unbind().click(function () {
            var vThisParentId = $(this).parents("li").attr("id");
        ...
        $("#prod_pricecomp .comparison__select > li").each(function (index, obj) {
            ...
            $.each(selShopArray, function (index) {
                $(_this).find(".lay-mallsel .lay-mall__list input:checkbox[value='" + selShopArray[index] + "']").prop("checked", true);
            });

            $(this).find(".lay-mallsel .lay-mall__list input:checkbox[value='all']").unbind().click(function () {
                if ($(_this).find("input:checkbox[value='all']").is(":checked")) {
                    $(_this).find("input:checkbox[value!='all']").prop("checked", true);
                } else {
                    $(_this).find("input:checkbox[value!='all']").prop("checked", false);
                }
            });
            $(this).find(".lay-mallsel .lay-mall__list input:checkbox[value!='all']").unbind().click(function () {
                $(_this).find("input:checkbox[value='all']").prop("checked", false);
            });
            $(this).find(".lay-mallsel .btn__group .btn__apply").unbind().click(function () {
                var ShopCheck = $(_this).find("input:checkbox[value!='all']:checked");
                var selShopComma = "";
                $.each(ShopCheck, function (i, v) {
                    if (i > 0) selShopComma += ",";
