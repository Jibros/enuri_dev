...
<%
String relModelNo = ChkNull.chkStr(request.getParameter("relmodelno"), "") ; //연관상품
...
try{
	...
	if (relModelNo.length() > 0 && paramModelNo > 0) { //필수 파라미터 O
		...
		String totalModelNoArr [] = (paramModelNo + "," + relModelNo).split(",");
		...
		for ( int i = 0; i < totalModelNoArr.length; i++ ){ 
			...
			if (i == 0) { //해당상품
				...
				String strModelNm = Wide_List_Proc.getRelatedProdModelNm(intModelNo);
	...
	System.out.println("relatedProd API error : " +e.getLocalizedMessage());
}
%>