...
<%
	...
	*/
	...
	//카테고리 검색일경우는 list로변경.
	if(strCate.length()>0) strFrom = "list";
	
	// 탭(리스트유형) ( 0:전체 / 1:모델리스트 / 2:일반상품리스트 / 3:해외직구)
	int intTab = ChkNull.chkInt(request.getParameter("_t"), 1);
	// 모바일 전문몰 일반상품 탭 고정
	...
	// 페이지당 호출 갯수
	int pageGap = ChkNull.chkInt(request.getParameter("pagesize"), 10);
	//1페이지인경우 임시로 20개로 셋팅
	if(pageNum==1 && pageGap==10 ) pageGap = 20;
	// 정렬
	/**
	...
	*/
	...
	String strMobile = ChkNull.chkStr(request.getParameter("_m"),"Y");
		...
		if(goodsOrderList.size()>0) {
			...
			for(int i=0;i<goodsOrderList.size();i++) {
				...
				if(goods_data_map.containsKey(strTargetItemNo)) {
					...
					icnt++;
					//1페이지이면서 pageGap = 10 이면 빠지는거 채우기 위해 20개 가져와서 10개만 보여줌.
					if(pageGap==20 && icnt>10) break;
				}
			}
			...
			if(pricelist_data_info!=null && pricelist_data_info.length>0) {
				...
				for(int i=0; i<pricelist_data_info.length; i++) {
					...
					if(pricelist_data_info[i].getPl_no()>0) {
						...
						listItem.put("deliveryInfo", strDeliveryInfo);
						...
						String strLandingUrl = "http://www.enuri.com/move/Redirect.jsp?cmd=move_link&vcode="+intShopCode+"&pl_no="+String.valueOf(lngPlNo)+"&cate="+strCateCode;
						if (strMobile.equals("Y")){
							strLandingUrl = "http://m.enuri.com/mobilefirst/move.jsp?plno="+String.valueOf(lngPlNo)+"&vcode="+intShopCode; 
						} 
						...
						//listItem.put("landingurl", strUrl);
...
%>
<%@ include file="/wide/include/IncGroupModelListJsonKbcar.jsp"%>
<%
			// 재검색 옵션 ( 돋보기 )
			...
			if(goods_data!=null && goods_data.length>0) {
				for(int i=0;i<goods_data.length;i++) {
					...
					if(intModelNo==0) {
						/*listItem.put("iscatalog","N");
						strLandingUrl = "http://www.enuri.com/move/Redirect.jsp?plno="+String.valueOf(strPlNo);
						if (strMobile.equals("Y")){
							strLandingUrl = "http://m.enuri.com/detail.jsp?plno="+String.valueOf(strPlNo); 
						} */
					}else{
						listItem.put("iscatalog","Y");
						strLandingUrl = "http://www.enuri.com/detail.jsp?modelno="+String.valueOf(intModelNo);
						if (strMobile.equals("Y")){
							strLandingUrl = "http://m.enuri.com/detail.jsp?modelno="+String.valueOf(intModelNo);
						} 
					}
					listItem.put("landingurl", strLandingUrl);
		...
		if(strCate.length()>0){
			retMap.put("cate", "\""+strCate+"\"");
			retMap.put("catename", "\""+Keyword_Cate_Zum_Proc.Category_Name_One(strCate)+"\"");
		}else{
			...
			retMap.put("catename", "\""+Keyword_Cate_Zum_Proc.Category_Name_One(keywordCate)+"\"");
		}
