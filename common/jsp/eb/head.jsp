<%
	...
	if (request.getServletPath().trim().indexOf("Index") >= 0 && request.getServletPath().trim().indexOf("Tour_Index.jsp") < 0){
		strIsMain = "_main";
	}

	if(request.getServletPath().trim().indexOf("prizeEvent") > -1)
	    isEvent = true;
%>
<%if(!isEvent){%>
<link rel="stylesheet" type="text/css" href="<%=ConfigManager.ENURI_COMMON%>/common/css/eb/default<%=strIsMain%>.css">
<link rel="stylesheet" type="text/css" href="<%=ConfigManager.ENURI_COMMON%>/common/css/eb/common<%=strIsMain%>.css">
<%}%>
<%
...
%>
<!--[if IE 7]>
<link rel="stylesheet" type="text/css" href="<%=ConfigManager.ENURI_COMMON%>/common/css/eb/common7<%=strIsMain%>.css">
<![endif]-->
<%
	//}
%>
<script type="text/javascript" src="<%=ConfigManager.IMG_ENURI_COM%>/common/js/eb/ui.js?v=1.02"></script>
