...
function SocialMainLink(url){
	var socialWindow = window.open(url,"_blank");
}
//이슈뉴스영역
var newswin;
...
function cmdNewsGoPage(i,j){ //페이지 직접선택
	clearTimeout(newsTimer);
	document.getElementById("div_main_news_"+i+"_"+newsPage).style.display = "none";
	newsPage = j;
	document.getElementById("div_main_news_"+i+"_"+newsPage).style.display = "block";
}
...
function cmdNewsTab(tab, page, auto){
	...
	if(auto!=1){
		//else if(tab==3){ //블로그		insertLog(6813);
		if(tab==1){ ////뉴스
			insertLog(4996);
		}else if(tab==2){ //가이드
			insertLog(4994);
...
function cmdNewsGetContents(i,f,auto){
	...
	function showNewsContents(originalRequest){
		...
		if(document.getElementById("div_main_news_"+i+"_"+f)){
			document.getElementById("div_main_news_"+i+"_"+f).style.display = "block";
		}
		newsTab = i;
		newsPage = f;
		if(auto==1 && f==1){
...
function cmdGoPopFashion(shopcode,plno,url){
	var win = window.open("/_pre_detail_.jsp", "_blank");
	win.location.href = "/move/Redirect.jsp?type=ex&cmd=move_fashion&vcode="+shopcode+"&pl_no="+plno+"&notToday=Y&url="+url;
	win.focus();
}
...
function getGuide_Resent_Zzim(){
	if (document.getElementById("IFrameMyFavoriteList")){
		document.getElementById("IFrameMyFavoriteList").contentWindow.document.location.reload();
		/*
		if (typeof(document.getElementById("IFrameMyFavoriteList").contentWindow.setMyfavoritelistOrMytodayviewlist) == "function"){
			document.getElementById("IFrameMyFavoriteList").contentWindow.setMyfavoritelistOrMytodayviewlist();
		}
		*/
	}
}