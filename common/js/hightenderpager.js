...
function pager2(url, total, scale, pgScale, page, qryStr) 
{
	var pgStr = "";
	var forvar = 0;
	var totalPg = 0;
	...
	if(total > scale) {
		...
		if ((page - pgScale) > 0)
		{
			pgStr +="<a href='"+ url +"&pg="+ (page - pgScale) +"'>이전 "+ pgScale +"</a>&nbsp;&nbsp;"
		}
...
//alert((1 + pgClus) +"=="+(forvar + pgClus));
		...
		//last page
		if (totalPg-page > pgScale)
		{
			pgStr += "&nbsp;&nbsp;<a href='"+ url +"&pg="+ (page + pgScale) +"'>다음 "+ pgScale  +"</a>"
		};
	} else if(total > 0 && total <= scale) {
		pgStr += "<b><font size=2>1</font></b>";
	};
...
function pager3(url, total, scale, pgScale, page, qryStr) 
{
	var pgStr = "";
	var forvar = 0;
	var totalPg = 0;
	...
	if(total > scale) {
		...
		if ((page - pgScale) > 0)
		{
			pgStr +="<a href='"+ url +"&pg="+ (page - pgScale) +"'> "+ firstImg +"</a>&nbsp;&nbsp;"
		}

		//shows previous page link if first page is passed
		if(page > 1) {
			pgStr += "<a href='";
			pgStr += url+"&pg=" + (page-1) + qryStr;
			pgStr += "'>" + prevImg + "</a>&nbsp;&nbsp;";
		}
		/*else {
			pgStr += "&nbsp;";
...
//alert((1 + pgClus) +"=="+(forvar + pgClus));
		for(i = 1 + pgClus; i <= forvar + pgClus ; i++) {
			if(i <= totalPg) {
				if(i==page) {
					pgStr += "<b><font color='#ff0000'>"+i+"</font></b> ";
				} else {
					pgStr += "<a href='";
					pgStr += url+"&pg=" + i + qryStr;
					pgStr += "'>" + i + "</a> ";
		...
		if (totalPg-page > pgScale)
		{
			pgStr += "&nbsp;&nbsp;<a href='"+ url +"&pg="+ (page + pgScale) +"'> "+ endImg +"</a>"
		};
	} else if(total > 0 && total <= scale) {
		pgStr += "<b><font size=2>1</font></b>";
	};
