...
function paging(nCurrentPage , nRecordSize, nBlockSize, nTotalRecordSize , target,fName){
    ...
    this.getPaging = function(){
        ...
        if(nTotalRecordSize > nRecordSize){
        	html += "<li><a class='pitem prev' num='" + prev + "'></a></li>";
        }
        
        for(i=this.nStartPage; i<=this.nEndPage; i++){
            html += "<li " + (nCurrentPage == i ? "class='on'" : "") + "><a class='pitem' num='" + i + "'>" + i + "</a></li>";
        }
        
        if(nTotalRecordSize > nRecordSize){
        	html += "<li><a class='pitem next' num='" + next + "'></a></li>"; 
        }
        ...
        for(i=0;i<obj.getElementsByTagName('li').length;i++){
            obj.getElementsByTagName('a')[i].onclick = function(){
                ...
                for(j=0;j<obj.getElementsByTagName("li").length;j++){
					...
					if(j==0){
						obj.getElementsByTagName("li")[j].className = "pitem prev";
					}else if(j==obj.getElementsByTagName('li').length-1){
						obj.getElementsByTagName("li")[j].className = "pitem next"; 
					}else{
						obj.getElementsByTagName("li")[j].className  = "pitem";
					}
                }  
            }
        }
