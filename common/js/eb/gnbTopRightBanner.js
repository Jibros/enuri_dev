...
jQuery(function(){
		...
		jQuery.getJSON("http://ad-api.enuri.info/enuri_PC/pc_home/B1/bundle?bundlenum=6",function(jsonObj){
			try {
				...
				jQuery.each(jsonObj,function(i,o){
		    		...
		    		arrHtml.push("	<a href=\"javascript:goBannerLink('" + o["TARGET"] +"', '"+o["JURL1"]+"');\">");
		    		arrHtml.push("		<img src=\""+o["IMG1"]+"\">");
		    		arrHtml.push("	</a>");
		    		arrHtml.push("</div>");
		    	});
		    	...
		    	jQuery("#bannerList").html(arrHtml.join("")).find("> div:eq(" + currImgNum + ")").show();
    ...
    //	insertLog(12472);
    	...
    	jQuery("#bannerList > div:eq(" + currImgNum + ")").show().siblings().hide();
    }
});