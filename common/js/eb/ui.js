...
jQuery(document).ready(function($){
	$(function(){
		...
		**/
		var _bMain = false;
		if (typeof(_thisismain) != "undefined"){
			_bMain = true;
		}
		...
		function gnb() {
			var _depth1 = $('#gnb > ul > li'),
				...
				digitDepth = $('#gnb > ul > li:nth-child(1)');
				digitDepth.find('.depth2').css('margin-left','0px');

				fashionDepth = $('#gnb > ul > li:nth-child(6)');
				fashionDepth.find('.depth2').css('margin-left','-8px');

				tourDepth = $('#gnb > ul > li:nth-child(7)');
				var bEvent = false;
				var _depthLm = -138;
				if (document.URL.indexOf("prizeEventMain.jsp") >= 0){
						bEvent = true;
				}
				if(bEvent){
					_depthLm = -135;
				}
				tourDepth.find('.depth2').css('margin-left',_depthLm+'px');
			...
			*/
			$('#gnb > ul > li > a').bind('mouseenter focusin', function(){
				...
				/*
    			if (!(location.pathname == "/" || location.pathname == "/Index.jsp" || location.pathname == "/Index1002.jsp")){
    			    var _ie7 = 2;
				...
				if (_href != null && _href != "" && _href.length > 4){
					...
					if(_cate_idx>0){
						...
						if (_seq == 1){
							...
							if(_bMain){
								gnb_log_cate(10786,_cate);
							}
						}else if(_seq == 2){
							gnb_log_cate(10540,_cate);
							if(_bMain){
								gnb_log_cate(10787,_cate);
							}
						}else if(_seq == 3){
							gnb_log_cate(10541,_cate);
							if(_bMain){
								gnb_log_cate(10788,_cate);
							}
						}else if(_seq == 4){
							gnb_log_cate(10542,_cate);
							if(_bMain){
								gnb_log_cate(10789,_cate);
							}
						}else if(_seq == 5){
							gnb_log_cate(10543,_cate);
							if(_bMain){
								gnb_log_cate(10790,_cate);
							}
						}else if(_seq == 6){
							gnb_log_cate(10544,_cate);
							if(_bMain){
								gnb_log_cate(10791,_cate);
							}
						}else if(_seq == 4778){
							gnb_log_cate(11167,_cate);
							if(_bMain){
								gnb_log_cate(11722,_cate);
							}
						}
					}else{
						if(_seq == 4778){
			...
			$('.depth3DetailBtn > a').click(function(){
				...
				_depth3Detail.show();
				/*가로폭*/
				if (getBrowserName() == "msie7" && document.URL.indexOf("Index.jsp") < 0 && document.URL.indexOf("Index1002.jsp") < 0){
				...
				if($("#depth3Detail_"+_seq).html()==""){
							...
							$.ajax({
								...
								success: function(data) {
									...
									//}else{
										...
										if(_seq!=4778) $("#depth3Detail_4778").hide();
										if(_seq!=4778) $("#depth3Detail_4778").html("");
					...
					if(_seq!=4778) $("#depth3Detail_4778").hide();
					if(_seq!=4778) $("#depth3Detail_4778").empty("");
				}
			...
			$('#gnb').bind('mouseleave', function(){
				...
				//stopGnbBannerRoll();
				if (typeof(initGnbImage) == "function"){
					initGnbImage();
				}
			});
		...
		$.ajax({
			...
			success: function(data) {
				...
				if (getBrowserName() == "msie7" && _bIndex && navigator.userAgent.toLowerCase().indexOf("trident/4.0") <0 ){
					$('.depth2allMenu > ul > li:nth-child(5)').css("width","199");
					$('.depth2allMenu > ul > li:nth-child(10)').css("width","199");
					$('.depth2allMenu > ul > li:nth-child(5) > ul').css("background-color","#ffffff");
					$('.depth2allMenu > ul > li:nth-child(10) > ul').css("background-color","#ffffff");
				}
				/*
