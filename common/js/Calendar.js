...
function opencalendar(temptargetfield) {
  var obj= document.getElementById(temptargetfield);
  Calendar_D(obj);
}
...
function Calendar_Click(e) {
	cal_Day = e.title;
	cal_DayOfWeek = e.week;
	if (cal_Day.length > 6) {
		target.value = cal_Day;
		if(targetWeek != null) targetWeek.value = cal_DayOfWeek;
	}
	oCalendarPopup.hide();
}
...
function Show_cal(sYear, sMonth, sDay) {
	...
	for (var optYear=(intThisYear-10); optYear<(intThisYear+11); optYear++) {
		...
		for (intLoopDay=1; intLoopDay <= 7; intLoopDay++) {	// 요일단위 루프 시작, 일요일 부터
			if (intThirdWeekday > 0) {											// 첫주 시작일이 1보다 크면
				...
				if (thirdPrintDay > intLastDay) {								// 입력 날짝 월말보다 크다면
					...
					Cal_HTML += "<td onClick=parent.Calendar_Click(this); ";
					Cal_HTML += "title="+intThisYear+"-"+day2(intThisMonth).toString()+"-"+day2(thirdPrintDay).toString()+" ";
					Cal_HTML += "week="+(intLoopDay-1).toString()+" ";
	...
	var oCalendarPopupBody = oCalendarPopup.document.body;
	oCalendarPopupBody.style.backgroundColor = "lightyellow";
	oCalendarPopupBody.style.border = "solid black 1px";
	oCalendarPopupBody.innerHTML = Cal_HTML;

	var calHeight = oCalendarPopupBody.document.all.Cal_Table.offsetHeight;
	//행이 6개 행인지, 5개인지 구분
	if (intLoopWeek == 6)	calHeight = 214;
	else	calHeight = 189;
	
	oCalendarPopup.show(pop_left, (pop_top + target.offsetHeight), 170, calHeight, document.body);
}
...
function Show_cal_M(sYear, sMonth) {
	...
	oCalendarPopup.show(pop_left, (pop_top + target.offsetHeight), 160, 99, document.body);
}
