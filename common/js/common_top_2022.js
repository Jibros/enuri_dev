    ...
    function loadGnbAllMenu(){
    	...
    	if(allGnbObj==null){
	    	jQuery.ajax({
	    		...
	    		url : isDev ? "/common/jsp/Ajax_Gnb_AllMenu_2022.jsp" : "/wide/main/ajax/TopAllCurtainCate_2022.html",
	    		data : "html",
	    		success:function(data){
	    			allGnbObj = data;
					...
					var heightAdjValue = 0;
					var depth1Height = 414;
					var depth2Height = 0;
					var depth3Height = 0;
					var heightAdjTarget1 = jQuery('.lay-cate-all .cate--depth1');
					var heightAdjTarget2 = jQuery('.lay-cate-all .cate--depth2_bundle');
					var heightAdjTarget3 = jQuery('.lay-cate-all .cate--depth3');

					function compareSector() {
						if(depth1Height > depth2Height && depth1Height > depth3Height) {
							heightAdjValue = depth1Height;
						} else if (depth2Height > depth1Height && depth2Height > depth3Height) {
							heightAdjValue = depth2Height;
						} else if (depth3Height > depth1Height && depth3Height > depth2Height) {
							heightAdjValue = depth3Height;
						}
						heightAdjTarget1.css('height', heightAdjValue);
						heightAdjTarget2.css('height', heightAdjValue);
						heightAdjTarget3.css('height', heightAdjValue);
					}

					jQuery('body').on('click','.header-cate__btn--all', function(){
						$(this).addClass('is--on');
						$('.lay-cate-all').removeClass('is--hide').before('<div class="lay-cate-all-overlay"></div>')
						depth2Height = $('.cate-item--depth1.active .cate--depth2_bundle').outerHeight();
						compareSector()
					...
					jQuery('body').on('mouseenter','.lay-cate-all li', function(){
						if(!jQuery(this).hasClass('cate-item--depth3')) {
							heightAdjTarget1.css('height', 'auto');
							heightAdjTarget2.css('height', 'auto');
							heightAdjTarget3.css('height', 'auto');
						}
						if(jQuery(this).hasClass('cate-item--depth1')) {
							jQuery(this).addClass('active').siblings().removeClass('active');
							depth1Height = heightAdjTarget1.outerHeight();
							depth2Height = jQuery(this).find('.cate--depth2_bundle').outerHeight();
							if(depth2Height == undefined) depth2Height = 0;

							if(jQuery(this).find('.cate-item--depth2.active').length == 0) depth3Height = 0;
							else depth3Height = jQuery(this).find('.cate-item--depth2.active .cate--depth3').outerHeight();
							if(depth3Height == undefined) depth3Height = 0;

							compareSector()
						}
						if(jQuery(this).hasClass('cate-item--depth2')){
							jQuery('.cate-item--depth2').removeClass('active');
							jQuery(this).addClass('active');
							depth3Height = jQuery(this).find('.cate--depth3').outerHeight();
							if(depth3Height == undefined) depth3Height = 0;
							compareSector();
						}
						if(jQuery(this).hasClass('cate-item--depth3')) {
						...
						// console.log('적용높이 : '+ heightAdjValue +' / ' + '1높이 :' + depth1Height + ' / ' + '2높이 :'+ depth2Height + ' / ' + '3높이 :' + depth3Height );
					})
					jQuery(window).on('scroll', function(){
						jQuery('.header-cate__btn--all').removeClass('is--on');
						jQuery('.lay-cate-all').addClass('is--hide');
						jQuery('.lay-cate-all-overlay').remove();
					});
					compareSector();
                    topSelectCate();
    ...
    function topSelectCate(){

    	var txt = jQuery(".location__btn--cate.depth1").text().trim();
    	if( typeof txt == "undefined" || txt == null || txt == "" ){
    		
    	}else{
    		jQuery('#gnbAllMenu').find("li").removeClass('active');
    		jQuery("#gnbAllMenu > li.cate-item--depth1").each(function(i,v){
        		var tit = $(this).find("div.cate__tit").text();
        		if(txt == tit){
